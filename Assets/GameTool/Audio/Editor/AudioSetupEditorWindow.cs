...
namespace GameTool.Audio.Editor
{
    public class AudioSetupEditorWindow : EditorWindow
    {
        ...
        public static void UpdateScripts(AudioTable instance)
        {
            ...
            EditorUtils.UpdateEnumInFile(systemFilePath, "eSoundName",
                instance.soundTracksSerializers.Select(sound => sound.key).ToList(),
                typeof(eSoundName));
            EditorUtils.UpdateEnumInFile(systemFilePath, "eMusicName",
                instance.musicTracksSerializers.Select(music => music.key).ToList(),
                typeof(eMusicName));
