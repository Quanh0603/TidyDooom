...
namespace GameTool.UI.Scripts.ItemUI
{
    public class AddItemEffectManager : SingletonMonoBehaviour<AddItemEffectManager>
    {
        ...
        [Header("SPEND EFFECT")] [SerializeField]
        PositionShowItemSpendEffect[] listPosShowItemSpendEffect;
        ...
        public void ShowItemsSpriteToPos(ItemResourceType itemType, Transform startPosTrans, Transform endPosTrans,
            int amount, float startScale = 1f, float endScale = 1f, float startAngle = 0f, float endAngle = 0f,
            float timeBetween = 0.1f, float timeMove = 10f, Action callbackOne = null, Action callback = null)
        {
            StartCoroutine(WaitEffectItemSpriteToPos(itemType, startPosTrans, endPosTrans, amount, startScale, endScale,
                startAngle, endAngle, timeBetween, timeMove, callbackOne, callback));
        }

        IEnumerator WaitEffectItemSpriteToPos(ItemResourceType itemType, Transform startPosTrans, Transform endPosTrans,
            int amount, float startScale = 1f, float endScale = 1f, float startAngle = 0f, float endAngle = 0f,
            float timeBetween = 0.1f, float timeMove = 10f, Action callbackOne = null, Action callback = null)
        {
            if (amount > 0)
            {
                ...
                for (int i = 0; i < amount; i++)
                {
                    ...
                    ItemShowSpriteController itemShowSpriteController = PoolingManager.Instance
                        .GetObject(ePrefabPool.ItemShowSpritePrefab, null, startPos, Vector3.one * startScale,
                            Quaternion.Euler(0f, 0f, startAngle)).GetComponent<ItemShowSpriteController>();
                    ...
                    itemShowSpriteController.transform
                        .DORotate(new Vector3(0f, 0f, endAngle), 100f, RotateMode.FastBeyond360).SetSpeedBased(true)
                        .SetEase(Ease.Linear);
                    itemShowSpriteController.transform
                        .DOPath(new Vector3[] { startPos, posPath, endPos }, timeMove, PathType.CatmullRom)
                        .SetEase(animationCurveSprite).OnComplete(() =>
                        {
                            callbackOne?.Invoke();
        ...
        public void ShowItemsUI(ItemResourceType itemType, int valueReward, int amountItem = 10, float startScale = 1f,
            float endScale = 1f, float startAngle = 0f, float endAngle = 0f, float timeBetween = 0.1f,
            float timeMove = 1f, Action callbackOne = null, Action callback = null, bool effectItemDone = true,
            bool canSetData = true)
        {
            ...
            ItemShowUIController itemRewardUIShow = PoolingManager.Instance
                .GetObject(ePrefabPool.ItemRewardUIShow, posRewardUI.transform).GetComponent<ItemShowUIController>();
            ...
            {
                StartCoroutine(AnimateItemUI(itemType, itemRewardUIShow.transform, valueReward, amountItem, startScale,
                    endScale, startAngle, endAngle, timeBetween, timeMove, callbackOne, callback, effectItemDone));
                itemRewardUIShow.transform.DOKill();
            });
        }

        IEnumerator AnimateItemUI(ItemResourceType itemType, Transform itemRewardShowTrans, int valueItem,
            int amount = 10, float startScale = 1f, float endScale = 1f, float startAngle = 0f, float endAngle = 0f,
            float timeBetween = 0.1f, float timeMove = 1f, Action callbackOne = null, Action callback = null,
            bool effectItemDone = true)
        {
            if (amount > valueItem)
            {
                amount = valueItem;
            ...
            if (amount > 0)
            {
                ...
                for (int i = 0; i < amount; i++)
                {
                    ...
                    EffectItemUI(itemType, startPos, endPos, ePrefabPool, nameAudio, nameAudioMove, icon, count,
                        valueOfOneItem, surplus, amount, startScale, endScale, startAngle, endAngle, timeMove,
                        callbackOne, effectItemDone);
        ...
        public void ShowItemsUIWithStartPos(ItemResourceType itemType, Transform startPosTrans, int valueReward,
            int amountItem = 10, float startScale = 1f, float endScale = 1f, float startAngle = 0f, float endAngle = 0f,
            float timeBetween = 0.1f, float timeMove = 1f, Action callbackOne = null, Action callback = null,
            bool effectItemDone = true, bool canSetData = true)
        {
            if (valueReward <= 0) return;
            countShowUI++;
            ...
            StartCoroutine(AnimateItemUIWithStartPos(itemType, startPosTrans, valueReward, amountItem, startScale,
                endScale, startAngle, endAngle, timeBetween, timeMove, callbackOne, callback, effectItemDone));
        }

        IEnumerator AnimateItemUIWithStartPos(ItemResourceType itemType, Transform startPosTrans, int valueItem,
            int amount = 10, float startScale = 1f, float endScale = 1f, float startAngle = 0f, float endAngle = 0f,
            float timeBetween = 0.1f, float timeMove = 1f, Action callbackOne = null, Action callback = null,
            bool effectItemDone = true)
        {
        ...
        public void ShowItemsUIWithEndPos(ItemResourceType itemType, Transform endPosTrans, int valueReward,
            int amountItem = 10, float startScale = 1f, float endScale = 1f, float startAngle = 0f, float endAngle = 0f,
            float timeBetween = 0.1f, float timeMove = 1f, Action callbackOne = null, Action callback = null,
            bool effectItemDone = true, bool canSetData = true)
        {
            ...
            itemRewardUIShow.transform.DOScale(1f, 0.2f).SetUpdate(true).SetEase(Ease.OutQuad).OnComplete(() =>
            {
                StartCoroutine(AnimateItemUIEndPos(itemType, itemRewardUIShow.transform, endPosTrans, valueReward,
                    amountItem, startScale, endScale, startAngle, endAngle, timeBetween, timeMove, callbackOne,
                    callback, effectItemDone));
                itemRewardUIShow.transform.DOKill();
            });
        }

        IEnumerator AnimateItemUIEndPos(ItemResourceType itemType, Transform itemRewardShowTrans, Transform endPosTrans,
            int valueItem, int amount = 10, float startScale = 1f, float endScale = 1f, float startAngle = 0f,
            float endAngle = 0f, float timeBetween = 0.1f, float timeMove = 1f, Action callbackOne = null,
            Action callback = null, bool effectItemDone = true)
        {
            if (amount > valueItem)
            {
                amount = valueItem;
        ...
        public void ShowItemsUIWithPos(ItemResourceType itemType, Transform startTrans, Transform endTrans,
            int valueReward, int amountItem = 10, float startScale = 1f, float endScale = 1f, float startAngle = 0f,
            float endAngle = 0f, float timeBetween = 0.1f, float timeMove = 1f, Action callbackOne = null,
            Action callback = null, bool effectItemDone = true, bool canSetData = true)
        {
            if (valueReward <= 0) return;
            countShowUI++;
            if (canSetData)
            ...
            StartCoroutine(AnimateItemUIWithPos(itemType, startTrans, endTrans, valueReward, amountItem, startScale,
                endScale, startAngle, endAngle, timeBetween, timeMove, callbackOne, callback, effectItemDone));
        }

        IEnumerator AnimateItemUIWithPos(ItemResourceType itemType, Transform startTrans, Transform endTrans,
            int valueItem, int amount = 10, float startScale = 1f, float endScale = 1f, float startAngle = 0f,
            float endAngle = 0f, float timeBetween = 0.1f, float timeMove = 1f, Action callbackOne = null,
            Action callback = null, bool effectItemDone = true)
        {
        ...
        void EffectItemUI(ItemResourceType itemType, Vector3 startPos, Vector3 endPos, ePrefabPool namePrefabBool,
            eSoundName nameAudio, eSoundName nameAudioMove, Sprite icon, int count, int valueOfOneItem, int surplus,
            int amount = 10, float startScale = 1f, float endScale = 1f, float startAngle = 0f, float endAngle = 0f,
            float timeMove = 1f, Action callbackOne = null, bool effectItemDone = true)
        {
            float randomX = 0f;
            float randomY = 0f;
            float timeFade = 0.5f;
            ...
            ItemShowUIController itemShowUIController = PoolingManager.Instance
                .GetObject(namePrefabBool, canvasUI.transform, startPos, Vector3.one * startScale,
                    Quaternion.Euler(0f, 0f, startAngle)).GetComponent<ItemShowUIController>();
            ...
            itemShowUIController.transform.DORotate(new Vector3(0f, 0f, endAngle), 100f, RotateMode.FastBeyond360)
                .SetUpdate(true).SetSpeedBased(true).SetEase(Ease.Linear);
            itemShowUIController.transform.DOScale(endScale, 1f).SetEase(Ease.Linear).SetUpdate(true);

            if (typeEffectUI == TypeEffect.Type1)
            {
                itemShowUIController.transform
                    .DOPath(new[] { itemShowUIController.transform.position, newPosMove, endPos }, timeMove,
                        PathType.CatmullRom).SetUpdate(true).SetEase(animationCurveUI).OnComplete(() =>
                    {
                        SetDataDoneEffectItemUI(itemType, count, amount, valueOfOneItem, surplus, nameAudio,
                            itemShowUIController, callbackOne, effectItemDone);
                    });
            }
            else if (typeEffectUI == TypeEffect.Type2)
            {
                itemShowUIController.transform.DOMove(newPosMove, 0.2f).SetEase(Ease.OutBack).SetUpdate(true)
                    .OnComplete(() =>
                    {
                        Vector3 posCenter = CalculateMidPoint(newPosMove, endPos, randomX >= 0 ? 2f : -2f);

                        itemShowUIController.transform
                            .DOPath(new[] { newPosMove, posCenter, endPos }, timeMove, PathType.CatmullRom)
                            .SetDelay(0.2f).SetUpdate(true).SetEase(Ease.InBack).OnComplete(() =>
                            {
                                SetDataDoneEffectItemUI(itemType, count, amount, valueOfOneItem, surplus, nameAudio,
                                    itemShowUIController, callbackOne, effectItemDone);
                            });
        ...
        void SetDataDoneEffectItemUI(ItemResourceType itemType, int i, int amount, int valueOfOneItem, int surplus,
            eSoundName nameAudio, ItemShowUIController itemShowUIController, Action callbackOne = null,
            bool effectItemDone = true)
        {
            switch (itemType)
            {
                ...
                case ItemResourceType.Diamond:
                    if (i == amount - 1)
                    {
                        GameToolSample.GameDataScripts.Scripts.GameData.Instance.DiamondFake +=
