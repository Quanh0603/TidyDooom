...
namespace GameTool.Editor
{
    public class InspectorHelperEditor
    {
        ...
        private static void CachedData(GameObject root)
        {
            ...
            saveCache.Clear();
            var rootCache = new TransformCache()
                { trans = root.transform, parent = null, level = 0, baseName = CleaningName(root.name) };
            ...
            if (!EditorSceneManager.IsPreviewScene(activeGameObject.scene))
            {
                if (PrefabUtility.IsPartOfPrefabInstance(activeGameObject))
                {
                    ...
                    if (prefabParent != null)
                    {
                        ...
                        PrefabUtility.SaveAsPrefabAssetAndConnect(activeGameObject, prefabPath,
...
//}
    public class CopyComponentsWindows : EditorWindow
    {
        ...
        {
            sourceObject =
                (GameObject)EditorGUILayout.ObjectField("Source Object", sourceObject, typeof(GameObject), true);
            targetObject =
            ...
            if (GUILayout.Button("Copy Components"))
            {
                if (sourceObject != null && targetObject != null)
                {
                    ...
                    foreach (Component component in components)
                    {
                        ...
                        FieldInfo[] fields = targetCom.GetType()
                            .GetFields(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
                        // Duyệt qua từng field
                        foreach (FieldInfo field in fields)
                        {
